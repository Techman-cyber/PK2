...
translations = {
    ...
    window.openProfileModal = () => {
        ...
        if(editName) editName.value = localStorage.getItem('patukrishi_user_name') || '';
    };
    ...
    window.saveProfile = () => {
        ...
        if(nameInput && nameInput.value) {
            localStorage.setItem('patukrishi_user_name', nameInput.value);
            const welcomeName = document.getElementById('welcome-name');
            if(welcomeName) welcomeName.innerText = nameInput.value;
            const headerName = document.getElementById('header-name');
            if(headerName) headerName.innerText = nameInput.value;
            const headerAvatar = document.getElementById('header-avatar');
            if(headerAvatar) headerAvatar.innerText = nameInput.value.charAt(0).toUpperCase();
            window.closeProfileModal();
